  ...
  return (
    <div className="page">
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6 }}>
            {done} of {relevant.length} documents ready
          </div>
      ...
      <div style={{ background: 'var(--card-bg)', borderRadius: 14, overflow: 'hidden', marginBottom: 16 }}>
        {relevant.map(dt => {
          const isDone = !!records[dt.id]?.ready;
          return (
            <div key={dt.id} onClick={() => handleToggle(dt.id, !isDone)}
              style={{
                display: 'flex', alignItems: 'flex-start', gap: 14,
                padding: '16px 20px', borderBottom: '1px solid var(--gold-bg)',
                cursor: 'pointer',
              }}
            ...
            >
              <div style={{
                ...
                border: `2px solid ${isDone ? 'var(--near-black)' : 'var(--gold)'}`,
                background: isDone ? 'var(--near-black)' : 'transparent',
                display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s',
              }}>
                {isDone && <svg width="11" height="9" viewBox="0 0 11 9" fill="none"><path d="M1 4L4 7.5L10 1" stroke="#f5edd9" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 500, color: isDone ? 'var(--text-muted)' : 'var(--near-black)', textDecoration: isDone ? 'line-through' : 'none' }}>
                  {dt.name}
                </div>
                {dt.hint && <div style={{ fontSize: 12, color: 'var(--text-light)', marginTop: 2, lineHeight: 1.4 }}>{dt.hint}</div>}
              </div>
              <span style={{
                fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 20, whiteSpace: 'nowrap', flexShrink: 0,
                background: isDone ? 'var(--success-bg)' : 'var(--gold-card)',
                color: isDone ? 'var(--success)' : 'var(--gold-dark)',
              }}>
                {isDone ? '✓ Ready' : 'Mark ready'}
              </span>
            </div>
          );
        })}
